...
const filteredComponents = components.filter(item => ['c-column', 'c-container', 'c-content', 'c-row', 'c-field', 'c-list', 'user-repos'].indexOf(item.name) === -1);

[{ name: 'All' }, ...categories].map(category => renderKinds(category, filteredComponents, 'Components', renderContent));
...
function renderContent(item) {
  const content = contents.find(doc => doc.id === item.content);
  const name = item.name.replace(/^c-/, '');
  let template;
  let docs;
